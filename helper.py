import torch 
from utils import img2mse, mse2psnr, encode_position, render_image_depth, sample_pdf
import config
...
def eval_model( dataloader, coarseModel, fineModel ):
    ...
    with torch.no_grad():
        for image, originVectorCoarse, directionVectorCoarse, tValsCoarse in dataloader:
            ...
            coarseModel.eval()
            (rgbCoarse, sigmaCoarse) = coarseModel(raysCoarse, dirsCoarse)
            ...
            # Sets model to TRAIN mode
            fineModel.eval()
            (rgbFine, sigmaFine) = fineModel(raysFine, dirsFine)
